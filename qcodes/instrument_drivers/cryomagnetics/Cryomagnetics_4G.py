...
class Cryomagnetics_4G(VisaInstrument):
    ...
    def __init__(self, name: str, address: str, axes=('Z','Y'), baudrate=9600, reset=False,**kwargs,):
...
#        return ans
    ...
    def set_uplim(self, channel, val):
        self._select_channel(channel)
        self.write_custom('ULIM %f\n' % val)

    def _get_field(self, channel):
        self._select_channel(channel)
        ans = self._ask_custom('IMAG?')
        return ans
